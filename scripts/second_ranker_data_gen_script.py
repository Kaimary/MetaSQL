...
def main(
    ...
    """
    # Initialization
    preds = [line.strip() for line in open(model_output_file, 'r').readlines()] if os.path.exists(model_output_file) else None
    ...
    embedder = SentenceTransformer(
        DIR_PATH + RETRIEVAL_MODEL_DIR.format(dataset_name))
    ...
    with open(dataset_file, "r") as data_file:
        ...
        total_count = 0
        for index, ex in tqdm(enumerate(data)):
            total_count += 1
            db_id = ex['db_id']
            ...
            if preds:
                pred = sql_string_format(preds[index])
                sqls.append(pred)
