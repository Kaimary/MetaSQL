...
def compute_beam_idx(batch_size, seq_len, binary_op_count, unary_op_count, device):
    ...
    l_binary_beam_idx = torch.div(binary_beam_idx, seq_len, rounding_mode='trunc')
...
def batched_span_select(
    ...
    max_span_range_indices: torch.Tensor = util.get_range_vector(
    ...
    #   t  raw_span_indices = span_ends - max_span_range_indices
...
def replace_masked_values_with_big_negative_number(x: torch.Tensor, mask: torch.BoolTensor):
    """
...
def batched_index_add(
    ...
    target : `torch.Tensor`, required.
        A 3 dimensional tensor of shape (batch_size, indices.shape[1], embedding_size).
        This is the tensor need add to target.
    ...
    added_targets : `torch.Tensor`
        A tensor with shape [indices.size(), target.size(-1)] representing the embedded indices
        extracted from the batch flattened target tensor.
    """
    assert target.dtype == delta.dtype, "Target and delta must have same dtype"
    if flattened_indices is None:
        # Shape: (batch_size * d_1 * ... * d_n)
        flattened_indices = flatten_and_batch_shift_indices(indices, target.size(1))

    # Shape: (batch_size * sequence_length, embedding_size)
    flattened_target: torch.Tensor = target.view(-1, target.size(-1))
    flattened_delta = delta.view(-1, target.size(-1))

    # Shape: (batch_size * d_1 * ... * d_n, embedding_size)
    flattened_target.index_add_(0, flattened_indices, flattened_delta)
    return target
