...
def __CONVERT_COLUMN_NAME(orig, schema, use_table_semantics=False):
    if '.' not in orig: return orig   # star case
    column_semantics = schema['column_names_mapping'][orig].replace(' id', '')
    if use_table_semantics: column_semantics += f' of {__CONVERT_TABLE_NAME(orig.split(".")[0], schema)}'
    return column_semantics

def __CONVERT_OPERATOR_NAME(opt):
    # WHERE_OPS = ('not', 'between', '=', '>', '<', '>=', '<=', '!=', 'in', 'like', 'is', 'exists')
    if opt in [2, 10]: return ''
    elif opt == 1: return 'between'
...
def sql_to_phrases(sql: str, p_sql: Dict, schema: Dict, t2distincts: Dict, relation_tables: Dict):
    """
    SQL phrases:
    ...
    label, star_semantics = _dig_join_labels(p_sql['from'], schema, relation_tables, t2distincts)
    if p_sql['groupBy']:
        _, col, _= p_sql['groupBy'][0]
        normalized_col = col.strip('_')
        if normalized_col in schema['foreign_key_columns']:  # if foreign key, use the corresponding primary key instead
            gcol_t = schema['foreign_column_pairs'][normalized_col].split('.')[0]
        else: gcol_t = normalized_col.split('.')[0]
        if gcol_t in star_semantics.keys(): 
            star_semantics['global'] = star_semantics[gcol_t]
            # If GROUP exisits, change join semantics as group semantics instead
            label = star_semantics['global'] 
    ...
    if len(output) > 1:
        ...
        for tok in sql.split():
            tok = tok.strip(';') # Avoid last `;` character in certain instances
    ...
    condition_label = ""
    if p_sql['where']: 
        where_label = _dig_where_having_labels(p_sql['where'], schema, relation_tables, star_semantics, grouping_table, t2distincts)
        condition_label = where_label
    if p_sql['having']: 
        having_label = _dig_where_having_labels(p_sql['having'], schema, relation_tables, star_semantics, grouping_table, t2distincts)
    ...
    phrases, find_grouping_col, select_has_agg = \
        _dig_select_labels(p_sql['select'], schema, star_semantics, grouping_table, grouping_col, join_tables, phrases)
    ...
    if p_sql['intersect']:
        iue_label = _dig_iue_labels(p_sql['intersect'], p_sql['select'], schema, relation_tables, keyword=' and', t2distincts=t2distincts)
        condition_label += iue_label
    if p_sql['union']:
        iue_label = _dig_iue_labels(p_sql['union'], p_sql['select'], schema, relation_tables, keyword=' or', t2distincts=t2distincts)
        condition_label += iue_label
    if p_sql['except']:
        iue_label = _dig_iue_labels(p_sql['except'], p_sql['select'], schema, relation_tables, t2distincts=t2distincts)
        condition_label += iue_label
...
def _dig_join_labels(join: Dict, schema: Dict, relation_tables: Dict, t2distincts: Dict):
    label = ""
    star_semantics = {}
    ...
    key = ' JOIN '.join(sorted([t[1].strip('_') for t in join['table_units']]))
    ...
    if len(output) == 1:
        ...
        if G.number_of_nodes() == 3:
            if nx.is_isomorphic(G, TABLE_GRAPHS[0]):
                ...
                idx, idx1 = -1, -1
                for j, tup in enumerate(t2distincts[key]):
                    if tup[0] == subject_object[1]: idx1 = j
                    elif tup[0] == subject_object[0]: idx = j
                if idx != -1 and idx1 != -1:
                    star_semantics['global'] = __CONVERT_TABLE_SEMANTICS(subject_object[0], schema) \
                        if t2distincts[key][idx1] < t2distincts[key][idx] else __CONVERT_TABLE_SEMANTICS(subject_object[1], schema)
                else: star_semantics['global'] = __CONVERT_TABLE_SEMANTICS(relation, schema)
            ...
            idx, idx1 = -1, -1
            for j, tup in enumerate(t2distincts[key]):
                if tup[0] == nodes[1]: idx1 = j
                elif tup[0] == nodes[0]: idx = j
            if idx != -1 and idx1 != -1:
                star_semantics['global'] = __CONVERT_TABLE_SEMANTICS(nodes[1], schema) \
                    if t2distincts[key][idx] < t2distincts[key][idx1] else __CONVERT_TABLE_SEMANTICS(nodes[0], schema)
            else: star_semantics['global'] = __CONVERT_TABLE_SEMANTICS(label, schema)
            if len(nodes) == 2:
...
def translate_where_having(where_having, schema, relation_tables, star_semantics=None, star_semantics_key=None, use_table_semantics=False, t2distincts=None):
    ...
    for idx, condition in enumerate(where_having[::2]):
        ...
        _, col_name, _ = col_unit
        col_name = col_name.strip('_')
        col_name = star_semantics[star_semantics_key] if col_name == 'all' else __CONVERT_COLUMN_NAME(col_name, schema, use_table_semantics)
        if not isinstance(val1, Dict):
            ...
            if __CHECK_AGG_IN_SELECT(val1):
                ...
                if op_id in [2, 7]:
                    ...
                    from_label, _ = _dig_join_labels(val1["from"], schema, relation_tables, t2distincts)
                ...
                # If without where clause, use the table semantics to construct the label.
                    ...
                    from_, _ = _dig_join_labels(val1["from"], schema, relation_tables, t2distincts)
                    pred += f' {from_}'
                ...
                if subq['from'] == val1['from'] and not val1['where'] and not subq['where']:
                    from1, _ = _dig_join_labels(val1["from"], schema, relation_tables, t2distincts)
                    from2, _ = _dig_join_labels(subq["from"], schema, relation_tables, t2distincts)
                    ...
                    pred += f' {translate_where_having(val1["where"], schema, relation_tables, t2distincts)} {iue} {translate_where_having(subq["where"], schema, relation_tables, t2distincts)}'
            else:
...
def translate_order(order, limit, from_, schema, relation_tables, t2distincts):
    res = ""
    kw, output = __parse_order(order)
    # Only handle one-attribute order case, and ignore aggregation
    _, col_name = output[0]
    if col_name == 'all': 
        from_label, _ = _dig_join_labels(from_, schema, relation_tables=relation_tables, t2distincts=t2distincts)
        col_name = from_label
...
def _dig_where_having_labels(where: List, schema: Dict, relation_tables: Dict, star_semantics: Dict, star_semantics_key: str, t2distincts: Dict):
    where_label = ""
    predicate_dict = {}
    for condition, conjuction in zip(where[::2], [''] + where[1::2]):
        ...
        ## WHERE AirportCode NOT IN (SELECT SourceAirport FROM Flights UNION SELECT DestAirport FROM Flights)         airports which do not have any flight in and out
            ...
            if __CHECK_AGG_IN_SELECT(val1):
                ...
                order_label = f' {translate_order(val1["orderBy"], val1["limit"], val1["from"], schema=schema, relation_tables=relation_tables, t2distincts=t2distincts)}'
                if op_id in [2, 7]:
                ...
                if subq['from'] == val1['from'] and not val1['where'] and not subq['where']:
                    ...
                    label += f' {translate_where_having(val1["where"], schema, relation_tables, t2distincts)} {iue} {translate_where_having(subq["where"], schema, relation_tables)}'
            else:
                raise ValueError('Not supported nested query type' % val1)
            where_label += ' ' + conjuction + ' ' + label
            # phrases.append(label)
...
def _dig_order_labels(
    ...
    '''
    kw, output = __parse_order(order)
    if not output: return phrases, False
...
def _dig_select_labels(
    ...
    for col in output:
        ...
        if col_name in schema['foreign_key_columns']:
            # Make sure this is the real fk of the tables used in the sql
            fk_tables = schema['foreigns'][schema['foreign_key_columns'].index(col_name)].split('-')
            if all(t in join_tables for t in fk_tables): 
                label += ' [FK]'
...
def _dig_iue_labels(iue: Dict, select: Tuple, schema: Dict, relation_tables: Dict, keyword=' except', t2distincts=None):
    ...
    from_, star_semantics = _dig_join_labels(iue['from'], schema, relation_tables=relation_tables, t2distincts=t2distincts)
    grouping_table, _, _ = _dig_group_labels(iue['groupBy'], schema)

    label = keyword
    if iue['where']: label += f' {translate_where_having(iue["where"], schema, relation_tables, use_table_semantics=True)}'
    ...
    if not iue['where'] and not iue['groupBy'] and not iue['orderBy']:
        ...
        # if __CHECK_SELECT_CONSISTENT(iue['select'], select, schema): 
        #     label += f' {from_}'
        # else: 
        label += f' {translate_select(iue["select"], schema, star_semantics, grouping_table)} of {from_}'
    # phrases.append(label)
