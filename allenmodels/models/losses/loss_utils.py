...
def deterministic_neural_sort(s, tau, mask):
    ...
    n = s.size()[1]
    one = torch.ones((n, 1), dtype=torch.float64, device=dev)
    ...
    temp = [t.type(torch.float64) for t in temp]
    temp = [torch.cat((t, torch.zeros(n - len(t), device=dev))) for t in temp]
    scaling = torch.stack(temp).type(torch.float64).to(dev)  # type: ignore
