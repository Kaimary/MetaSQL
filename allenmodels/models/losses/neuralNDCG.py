...
from allenmodels.models.losses.loss_utils import DEFAULT_EPS, PADDED_Y_VALUE, get_torch_device
from allenmodels.models.losses.loss_utils import deterministic_neural_sort, sinkhorn_scaling, stochastic_neural_sort
from ..metrics.ndcg import dcg
import allennlp.nn.util as annu
...
def neuralNDCG(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE, temperature=1., powered_relevancies=True, k=None,
    ...
    if idcg_mask.all():
        return torch.tensor(0., device=dev, requires_grad=True)
...
def neural_ndcg_transposed(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE, temperature=1.,
                           powered_relevancies=True, k=None, stochastic=False, n_samples=32, beta=0.1,
                           log_scores=True, max_iter=50, tol=1e-6):
    """
...
def cross_entropy(y_pred, y_true, padded_value_indicator=PADDED_Y_VALUE):
    mask = (y_pred != padded_value_indicator)
    assert isinstance(mask, torch.Tensor)
    avg_scores = annu.masked_log_softmax(y_pred, mask.bool())
    # avg_scores = torch.nn.functional.log_softmax(y_pred, dim=-1)
    loss_mask = y_true == 1
    loss_tensor = -avg_scores * loss_mask.int()
    loss = loss_tensor.squeeze().sum(-1)
    return loss
