...
from nl2sql_models.gap.seq2struct.models import abstract_preproc, transformer
from nl2sql_models.gap.seq2struct.models.spider import spider_enc_modules
from nl2sql_models.gap.seq2struct.resources import pretrained_embeddings
from nl2sql_models.gap.seq2struct.utils import registry
from nl2sql_models.gap.seq2struct.utils import vocab
from nl2sql_models.gap.seq2struct.utils import serialization
from nl2sql_models.gap.seq2struct import resources
from nl2sql_models.gap.seq2struct.resources import corenlp
from transformers import BertModel, BertTokenizer, BartModel, BartTokenizer
from nl2sql_models.gap.seq2struct.models.spider.spider_match_utils import (
...
class SpiderEncoderBartPreproc(SpiderEncoderV2Preproc):
    ...
    def process_batch_items(self, items, section, validation_info):
        ...
        preproc_schema = self._preprocess_schema(items[0].schema)
        question = self._tokenize(items[0].text, items[0].orig['question'])
        question_bart_tokens = BartTokens(items[0].orig['question'], self.tokenizer)
        ...
        if self.compute_cv_link:
            cv_link = question_bart_tokens.bart_cv_linking(
                items[0].schema, self.db_path)
        else:
            cv_link = {"num_date_match": {}, "cell_match": {}}

        preprocessed = []
        for item in items:
            meta = self._tokenize(item.text, f"RATING: {item.orig['rating']}\nTAGS: {item.orig['tags']}\n{item.orig['flag']}\n")
            preprocessed.append(({
                'raw_question': item.orig['question'],
                'meta': meta,
                'question': question,
...
class SpiderEncoderBart(torch.nn.Module):
    ...
    def __init__(
        ...
        replace_model_with_pretrained(self.bert_model.encoder, os.path.join(
            "./nl2sql_models/gap/pretrained_checkpoint",
    ...
    def forward(self, descs):
        ...
        batch_id_map = {}  # some long examples are not included
        for batch_idx, desc in enumerate(descs):
            meta = self.pad_single_sentence_for_bert(desc['meta'], cls=True)
            qs = self.pad_single_sentence_for_bert(desc['question'], cls=False)
            if self.use_column_type:
                cols = [self.pad_single_sentence_for_bert(c, cls=False) for c in desc['columns']]
            else:
                cols = [self.pad_single_sentence_for_bert(c[:-1], cls=False) for c in desc['columns']]
            ...
            token_list = meta + qs + [c for col in cols for c in col] + \
                         [t for tab in tabs for t in tab]
            assert self.check_bert_seq(token_list)
            if len(token_list) > 512:
                long_seq_set.add(batch_idx)
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            assert q_enc.size()[0] == len(desc["question"]+desc["meta"])
            assert col_enc.size()[0] == c_boundary[-1]
            assert tab_enc.size()[0] == t_boundary[-1]
