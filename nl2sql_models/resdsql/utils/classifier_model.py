...
class MyClassifier(nn.Module):
    ...
    def table_column_cross_attention(
        ...
        for table_id in range(table_num):
            ...
            # print(f"table_name_embedding:{table_name_embedding.size()}")
            # print(f"column_info_embeddings_in_one_table:{column_info_embeddings_in_one_table.size()}")
            table_name_embedding_attn, _ = self.table_column_cross_attention_layer(
                table_name_embedding.view(len(table_name_embedding), 1, -1),
                column_info_embeddings_in_one_table.view(len(column_info_embeddings_in_one_table), 1, -1),
                column_info_embeddings_in_one_table.view(len(column_info_embeddings_in_one_table), 1, -1)
            )

            table_name_embedding_attn_list.append(table_name_embedding_attn.squeeze(1))
    ...
    def table_column_cls(
        ...
        for batch_id in range(batch_size):
            ...
            for table_name_ids in aligned_table_name_ids:
                ...
                # BiLSTM pooling
                output_t, (hidden_state_t, cell_state_t) = self.table_name_bilstm(table_name_embeddings.view(len(table_name_embeddings), 1, -1))
            ...
            # print(f"table_name_embeddings_in_one_db1:{table_name_embeddings_in_one_db.size()}")
            ...
            for column_info_ids in aligned_column_info_ids:
                ...
                # BiLSTM pooling
                output_c, (hidden_state_c, cell_state_c) = self.column_info_bilstm(column_info_embeddings.view(len(column_info_embeddings), 1, -1))
            ...
            # print(f"table_name_embeddings_in_one_db:{table_name_embeddings_in_one_db.size()}")
