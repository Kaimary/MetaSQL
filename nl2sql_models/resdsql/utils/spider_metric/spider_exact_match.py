...
from  nl2sql_models.resdsql.third_party.spider import evaluation as spider_evaluation
...
def compute_exact_match_metric(predictions, references) -> Dict[str, Any]:
    ...
    for prediction, reference in zip(predictions, references):
        ...
        if turn_idx < 0:
            continue
        a= evaluator.evaluate_one(reference["db_id"], reference["query"], prediction)
        tmp.append(a)
    evaluator.finalize()
    return {
