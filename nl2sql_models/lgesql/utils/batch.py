...
import torch
import numpy as np
from nl2sql_models.lgesql.utils.example import Example, get_position_ids
from nl2sql_models.lgesql.utils.constants import PAD, UNK
from nl2sql_models.lgesql.model.model_utils import lens2mask, cached_property
...
def from_example_list_base(ex_list, device='cpu', train=True):
    ...
    meta_lens = [len(ex.meta) for ex in ex_list]
    batch.meta_lens = torch.tensor(meta_lens, dtype=torch.long, device=device)
    ...
    if plm is None: # glove.42B.300d
        ...
        meta_mask_plm = [ex.meta_mask_plm + [0] * (max_len - len(ex.meta_mask_plm)) for ex in ex_list]
        batch.meta_mask_plm = torch.tensor(meta_mask_plm, dtype=torch.bool, device=device)
        ...
        # subword aggregation
        meta_subword_lens = [l for ex in ex_list for l in ex.meta_subword_len]
        batch.meta_subword_lens = torch.tensor(meta_subword_lens, dtype=torch.long, device=device)
    ...
    batch.meta_unk_mask, batch.question_unk_mask, batch.table_unk_mask, batch.column_unk_mask = None, None, None, None
...
class Batch():
    ...
    @cached_property
    def max_meta_len(self):
        return torch.max(self.meta_lens).item()
    ...
    @cached_property
    def max_meta_subword_len(self):
        return torch.max(self.meta_subword_lens).item()
    ...
    @cached_property
    def meta_mask(self):
        return lens2mask(self.meta_lens)
    ...
    @cached_property
    def meta_subword_mask(self):
        return lens2mask(self.meta_subword_lens)
