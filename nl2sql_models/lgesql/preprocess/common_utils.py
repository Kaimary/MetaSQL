...
class Preprocessor():
    ...
    def preprocess_question(self, entry: dict, db: dict, verbose: bool = False):
        ...
        flag = "CORRECT SOLUTION" if "flag" not in entry.keys() else entry["flag"]
        # meta = ';'.join([flag, f'rating: {entry["rating"]}', f'tags: {entry["tags"]}'])
        meta = ';'.join([flag])
        doc = self.nlp(meta)
        entry['raw_meta_toks'] = [w.text.lower() for s in doc.sentences for w in s.words]
        ...
        if verbose:
            # print('Metadata:', meta)
            # print('Tokenized:', ' '.join(entry['raw_meta_toks']))
