...
class GraphOutputLayerWithPruning(nn.Module):
    ...
    def forward(self, metas, inputs, batch):
        meta_outputs = metas.new_zeros(len(batch), batch.meta_mask.size(1), self.hidden_size)
        meta_outputs = meta_outputs.masked_scatter_(batch.meta_mask.unsqueeze(-1), inputs)
        outputs = inputs.new_zeros(len(batch), batch.mask.size(1), self.hidden_size)
        outputs = outputs.masked_scatter_(batch.mask.unsqueeze(-1), inputs)
        outputs = torch.cat((meta_outputs, outputs), 1)
        ...
        if self.training:
            ...
            return outputs, torch.cat((batch.meta_mask, batch.mask), 1), loss
        else:
            return outputs, torch.cat((batch.meta_mask, batch.mask), 1)

@Registrable.register('with_pruning_origin')
class GraphOutputLayerWithPruning1(nn.Module):

    def __init__(self, args):
        super(GraphOutputLayerWithPruning1, self).__init__()
        self.hidden_size = args.gnn_hidden_size
        self.graph_pruning = GraphPruning(self.hidden_size, args.num_heads, args.dropout, args.score_function)
