...
class GraphInputLayerPLM(nn.Module):
    ...
    def __init__(self, plm='bert-base-uncased', hidden_size=256, dropout=0., subword_aggregation='mean',
        ...
        self.plm_model = AutoModel.from_config(AutoConfig.from_pretrained(os.path.join('./nl2sql_models/lgesql/pretrained_models', plm))) \
            if lazy_load else AutoModel.from_pretrained(os.path.join('./nl2sql_models/lgesql/pretrained_models', plm))
    ...
    def forward(self, batch):
        ...
        meta, question, table, column = self.subword_aggregation(outputs, batch)
        input_dict = {
            "meta": self.dropout_layer(meta),
            "question": self.dropout_layer(question),
            "table": self.dropout_layer(table),
            "column": self.dropout_layer(column)
        }
...
class GraphInputLayerPLM1(nn.Module):

    def __init__(self, plm='bert-base-uncased', hidden_size=256, dropout=0., subword_aggregation='mean',
            schema_aggregation='head+tail', lazy_load=False):
        super(GraphInputLayerPLM1, self).__init__()
        self.plm_model = AutoModel.from_config(AutoConfig.from_pretrained(os.path.join('./nl2sql_models/lgesql/pretrained_models', plm))) \
            if lazy_load else AutoModel.from_pretrained(os.path.join('./nl2sql_models/lgesql/pretrained_models', plm))
        self.config = self.plm_model.config
        self.subword_aggregation = SubwordAggregation1(self.config.hidden_size, subword_aggregation=subword_aggregation)
        self.dropout_layer = nn.Dropout(p=dropout)
        self.rnn_layer = InputRNNLayer1(self.config.hidden_size, hidden_size, cell='lstm', schema_aggregation=schema_aggregation)
...
class SubwordAggregation(nn.Module):
    ...
    def forward(self, inputs, batch):
        ...
        """
        old_metas, old_questions, old_tables, old_columns = inputs.masked_select(batch.meta_mask_plm.unsqueeze(-1)), \
            inputs.masked_select(batch.question_mask_plm.unsqueeze(-1)), \
            inputs.masked_select(batch.table_mask_plm.unsqueeze(-1)), \
            inputs.masked_select(batch.column_mask_plm.unsqueeze(-1))
        metas = old_metas.new_zeros(batch.meta_subword_lens.size(0), batch.max_meta_subword_len, self.hidden_size)
        metas = metas.masked_scatter_(batch.meta_subword_mask.unsqueeze(-1), old_metas)
        ...
        metas = self.aggregation(metas, mask=batch.meta_subword_mask)
        ...
        new_metas, new_questions, new_tables, new_columns = metas.new_zeros(len(batch), batch.max_meta_len, self.hidden_size), \
        ...
        new_metas = new_metas.masked_scatter_(batch.meta_mask.unsqueeze(-1), metas)
        ...
        return new_metas, new_questions, new_tables, new_columns

class SubwordAggregation1(nn.Module):
    """ Map subword or wordpieces into one fixed size vector based on aggregation method
    """
    def __init__(self, hidden_size, subword_aggregation='mean-pooling'):
        super(SubwordAggregation1, self).__init__()
        self.hidden_size = hidden_size
        self.aggregation = PoolingFunction(self.hidden_size, self.hidden_size, method=subword_aggregation)
...
class InputRNNLayer(nn.Module):
    ...
    def forward(self, input_dict, batch):
        ...
        """
        metas, _ = rnn_wrapper(self.question_lstm, input_dict['meta'], batch.meta_lens, cell=self.cell)
        metas = metas.contiguous().view(-1, self.hidden_size)[lens2mask(batch.meta_lens).contiguous().view(-1)]
...
class InputRNNLayer1(nn.Module):
    def __init__(self, input_size, hidden_size, cell='lstm', schema_aggregation='head+tail', share_lstm=False):
        super(InputRNNLayer1, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.cell = cell.upper()
        self.question_lstm = getattr(nn, self.cell)(self.input_size, self.hidden_size // 2, num_layers=1, bidirectional=True, batch_first=True)
