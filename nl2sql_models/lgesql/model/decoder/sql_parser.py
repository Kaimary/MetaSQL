...
class SqlParser(nn.Module):
    ...
    def score(self, encodings, mask, h0, batch):
        ...
        split_len = [batch.max_meta_len, batch.max_question_len, batch.max_table_len, batch.max_column_len]
        _, q, tab, col = encodings.split(split_len, dim=1)
        _, q_mask, tab_mask, col_mask = mask.split(split_len, dim=1)
        if args.sep_cxt:
        ...
        for t in range(batch.max_action_num):
            ...
            # temperature = 1.2
            # apply_rule_logits = torch.div(self.apply_rule(self.apply_rule_affine(att_vec)).squeeze(1), temperature)
            # apply_rule_logprob = F.log_softmax(apply_rule_logits, dim=-1)
    ...
    def parse(self, encodings, mask, h0, batch, beam_size=5):
        ...
        while t < args.decode_max_step:
            ...
            # temperature = 1.2
            # apply_rule_logprob = torch.div(self.apply_rule(self.apply_rule_affine(att_vec)).squeeze(1), temperature)
            # apply_rule_logprob = torch.nn.functional.softmax(apply_rule_logprob, dim=-1)
...
@Registrable.register('decoder_tranx_origin')
class SqlParser1(nn.Module):
    def __init__(self, args, transition_system):
        super(SqlParser1, self).__init__()
        self.args = args
        self.transition_system = transition_system
        self.grammar = self.transition_system.grammar
